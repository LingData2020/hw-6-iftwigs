...
ggplot(upsid, aes(consonants, vowels)) + geom_point() + geom_line(data = augment(fit1), aes(consonants, .fitted), color = "blue")
```
...
ggplot(upsid, aes(consonants, vowels, color = area)) + geom_point() + stat_ellipse()
```
...
```{r 2.3, echo=FALSE}
ggplot(upsid, aes(consonants, vowels, color = area)) + geom_point() + geom_line(data = augment(fit2), aes(consonants, .fitted, color = area))
```
...
summary(fit2)
```
I am unable to interpret the changes because I don't see the p-value for the second model.About the lines it is possible to say that they are parallel and located at different heights, having various intercepts depending on their groups.
...
```{r 3.2, echo=FALSE}
ggplot(upsid, aes(consonants, vowels, color = area)) + geom_point() + geom_line(data=augment(fit3), aes(consonants, .fitted, color = area))
```
...
summary(fit3)
```
Again, I am unable to interpret the significance because I don't see the p-value. However it's possible to say that the correlation of fixed effects has changed. About the lines it is possible to say that they are not parallel anymore, must be due to the random effect applied to the slope.
...
```{r 4.2, echo=FALSE}
ggplot(upsid, aes(consonants, vowels, color = area)) + geom_point() + geom_line(data = augment(fit4), aes(consonants, .fitted, color = area))
```
...
summary(fit4)
```
I again do not see p-value here so I cannot answer. The difference between the lines might be caused by random effect applied to the intercept, slope and both of them in the different models.
